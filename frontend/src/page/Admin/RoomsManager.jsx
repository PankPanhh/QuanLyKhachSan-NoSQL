...
function RoomsManager() {
  ...
  const handleCloseModal = () => {
    ...
    if (selectedImagePreview) {
      try { URL.revokeObjectURL(selectedImagePreview); } catch (e) {}
      setSelectedImagePreview(null);
    }
    setSelectedImageFile(null);
  ...
  const handleSaveRoom = async (e) => {
    ...
    try {
      ...
      if (selectedImageFile) {
        const uploadResp = await adminUploadRoomImage(selectedRoom._id, selectedImageFile);
        returnedImageName = uploadResp && uploadResp.data && uploadResp.data.HinhAnh ? uploadResp.data.HinhAnh : null;
        // update local formData immediately so preview and UI reflect the new filename
        if (returnedImageName) {
          setFormData(prev => ({ ...prev, HinhAnh: returnedImageName, _imgVersion: Date.now() }));
        }
      ...
      setFilteredRooms(prev => prev.map(r => (r._id === updatedRoom._id ? updatedRoom : r)));
  ...
  return (
    <div className="container-fluid px-0"> {/* Loại bỏ các style dark-mode */}
              ...
              <div className="d-flex gap-2">
                <div className="dropdown">
                  ...
                  <ul className="dropdown-menu">
                    <li><a className="dropdown-item" href="#" onClick={() => handleBulkStatusUpdate('Trống')}>Sẵn sàng</a></li>
                    <li><a className="dropdown-item" href="#" onClick={() => handleBulkStatusUpdate('Đang dọn dẹp')}>Đang dọn</a></li>
                    <li><a className="dropdown-item" href="#" onClick={() => handleBulkStatusUpdate('Bảo trì')}>Bảo trì</a></li>
                    <li><a className="dropdown-item" href="#" onClick={() => handleBulkStatusUpdate('Hư')}>Hư hỏng</a></li>
                  </ul>
                </div>
                <Button
              ...
              <div className="card-body">
                <div className="card-title d-flex align-items-start justify-content-between">
                  <div className="avatar shrink-0">
                    <span className="avatar-initial rounded bg-label-success"><i className="bx bx-door-open"></i></span>
                  </div>
                </div>
                <span className="fw-semibold d-block mb-1">Sẵn sàng</span>
                <h3 className="card-title mb-2">{filteredRooms.filter(r => r.TinhTrang === 'Trống').length}</h3>
              </div>
            </div>
          </div>
          <div className="col-lg-3 col-md-6">
            <div className="card">
              <div className="card-body">
                <div className="card-title d-flex align-items-start justify-content-between">
                  <div className="avatar shrink-0">
                    <span className="avatar-initial rounded bg-label-info"><i className="bx bx-user-check"></i></span>
                  </div>
                </div>
                <span className="fw-semibold d-block mb-1">Có khách</span>
                <h3 className="card-title mb-2">{filteredRooms.filter(r => r.TinhTrang === 'Đang sử dụng').length}</h3>
              </div>
            </div>
          </div>
          <div className="col-lg-3 col-md-6">
            <div className="card">
              <div className="card-body">
                <div className="card-title d-flex align-items-start justify-content-between">
                  <div className="avatar shrink-0">
                    <span className="avatar-initial rounded bg-label-warning"><i className="bx bx-wrench"></i></span>
                  </div>
                </div>
                <span className="fw-semibold d-block mb-1">Bảo trì/Dọn</span>
                <h3 className="card-title mb-2">{filteredRooms.filter(r => ['Bảo trì', 'Hư', 'Đang dọn dẹp'].includes(r.TinhTrang)).length}</h3>
              </div>
            </div>
          </div>
           <div className="col-lg-3 col-md-6">
            <div className="card">
              <div className="card-body">
                <div className="card-title d-flex align-items-start justify-content-between">
                  <div className="avatar shrink-0">
                    <span className="avatar-initial rounded bg-label-primary"><i className="bx bx-calendar-check"></i></span>
                  </div>
                </div>
                <span className="fw-semibold d-block mb-1">Đã đặt</span>
                <h3 className="card-title mb-2">{filteredRooms.filter(r => r.TinhTrang === 'Đã đặt').length}</h3>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="text-center">
              ...
              {error && (
                <div className="mt-3">
                  ...
                  <Button className="btn btn-sm btn-outline-primary" onClick={fetchRooms}>Thử lại</Button>
                </div>
              )}
            </div>
          </div>
              ...
              <tbody className="table-border-bottom-0">
                {filteredRooms.length === 0 ? (
                  ...
                  filteredRooms.map((room, index) => {
                    ...
                    return (
                      <tr key={room._id} className={selectedRooms.includes(room._id) ? 'table-active' : ''}> 
                        <td>
                          <input 
                            className="form-check-input" 
                            type="checkbox" 
                        ...
                        <td>
                          <div className="d-flex align-items-center">
                            <img 
                              src={room._localPreview ? room._localPreview : (getRoomImageUrl(room.HinhAnh) + (room._imgVersion ? `?v=${room._imgVersion}` : ''))} 
                              alt={room.TenPhong}
                              className="rounded me-2"
                              style={{width: '30px', height: '30px', objectFit: 'cover'}}
                              onError={(e) => { e.target.src = "/assets/img/avatars/default.png"; }}
                            />
                        ...
                        <td>{room.TenPhong}</td>
                        <td>
                          <span className="badge bg-label-primary me-1">{room.LoaiPhong}</span>
                        </td>
                        <td>
                          <span className="fw-semibold text-dark">{formatCurrency(room.GiaPhong)}</span>
                        </td>
                        ...
                        <td>
                          <div className="dropdown">
                            ...
                            <ul className="dropdown-menu">
                              <li><a className="dropdown-item" href="#" onClick={() => handleQuickStatusUpdate(room._id, 'Trống', room.MaPhong)}>Sẵn sàng</a></li>
                              <li><a className="dropdown-item" href="#" onClick={() => handleQuickStatusUpdate(room._id, 'Đang dọn dẹp', room.MaPhong)}>Đang dọn</a></li>
                              <li><a className="dropdown-item" href="#" onClick={() => handleQuickStatusUpdate(room._id, 'Đang sử dụng', room.MaPhong)}>Có khách</a></li>
                              <li><a className="dropdown-item" href="#" onClick={() => handleQuickStatusUpdate(room._id, 'Bảo trì', room.MaPhong)}>Bảo trì</a></li>
                              <li><a className="dropdown-item" href="#" onClick={() => handleQuickStatusUpdate(room._id, 'Hư', room.MaPhong)}>Hư hỏng</a></li>
                            </ul>
                          </div>
                        </td>
                        <td>
          ...
          <div className="row g-3">
            ...
            <div className="col-md-6">
              <div className="mb-3">
                ...
                <img 
                  src={(formData.HinhAnh ? getRoomImageUrl(formData.HinhAnh) : getRoomImageUrl('default.jpg')) + (formData._imgVersion ? `?v=${formData._imgVersion}` : '')} 
                  alt="Xem trước"
              ...
              />
              <div className="mb-3">
                <label className="form-label">Upload ảnh mới (ghi đè file hiện có)</label>
                <input
                  type="file"
                  accept="image/*"
                  className="form-control"
                ...
                {selectedImagePreview && (
                  <div className="mt-2">
                    <img src={selectedImagePreview} alt="preview" className="img-fluid rounded" style={{height: '120px', objectFit: 'cover'}} />
                  </div>
                )}
              </div>
              <div className="mt-3">
          ...
          <div>
            <div className="row">
              <div className="col-md-6 mb-3">
                <img 
                  src={getRoomImageUrl(selectedRoom.HinhAnh) + (selectedRoom._imgVersion ? `?v=${selectedRoom._imgVersion}` : '')} 
              ...
              <div className="col-md-6">
                <span className={`badge ${getStatusColor(selectedRoom.TinhTrang).bg} mb-2`}>
                ...
                <div className="list-group list-group-flush">
                  ...
                  <div className="list-group-item d-flex justify-content-between px-0">
                    ...
                    <span className="fw-semibold">{selectedRoom.LoaiPhong}</span>
                  </div>
                   <div className="list-group-item d-flex justify-content-between px-0">
                    <span className="text-muted">Giá phòng</span>
                    <span className="fw-semibold text-success">{formatCurrency(selectedRoom.GiaPhong)}</span>
                  </div>
                  <div className="list-group-item d-flex justify-content-between px-0">
                  ...
                  <div className="list-group-item d-flex justify-content-between px-0">
                    ...
                    <span>{selectedRoom.SoGiuong} ({selectedRoom.LoaiGiuong || 'N/A'})</span>
                  </div>
                  <div className="list-group-item d-flex justify-content-between px-0">
